...
function ensureOvulationPoint(childIndex, birthDate) {
    ...
    // 1. Eisprung-Punkt NUR für die Schwangerschafts-Daten (Privat)
    if (!pregnancyData[childIndex]) pregnancyData[childIndex] = [];
    let hasZeroPreg = pregnancyData[childIndex].some(item => Number(item.size) === 0 && Number(item.weight) === 0);

    if (!hasZeroPreg) {
        pregnancyData[childIndex].unshift({ 
            date: ovulationDateStr, 
            size: "0", 
            weight: "0",
            note: "Start der Schwangerschaft (Eisprung)" 
        });
    ...
    // 2. Cleanup: Falls der 0-Punkt durch unseren letzten Test schon in der 
    // öffentlichen "Größe"-Tabelle (growthData) gelandet ist, löschen wir ihn hier heimlich wieder!
    if (growthData[childIndex]) {
        let originalLength = growthData[childIndex].length;
        // Ein Baby wird nie mit 0 cm und 0 kg geboren, also löschen wir diese Platzhalter
        growthData[childIndex] = growthData[childIndex].filter(item => !(Number(item.size) === 0 && Number(item.weight) === 0));
        
        if (growthData[childIndex].length !== originalLength) {
            localStorage.setItem('request_growth', JSON.stringify(growthData));
        }
    }
}
...
// ==========================================
// SHARE MODAL & WHATSAPP (Die 3 Szenarien)
// ==========================================
...
function shareAppViaWhatsApp(type) {
    let referralCode = "BABYSTEPS-10"; // Dein Code für Rabatte
    let familyToken = "FAM-" + Math.floor(Math.random() * 100000); // Simulierte eindeutige Familien-ID
    let text = "";
    
    if (type === 'partner') {
        text = encodeURIComponent(`Hallo! Lade dir die BabySteps-App herunter, damit wir unser Tagebuch gemeinsam führen können. Klicke auf diesen Link, um unserem Familien-Profil kostenlos beizutreten: https://deine-app-domain.de/join/${familyToken}`);
    } 
    else if (type === 'family') {
        text = encodeURIComponent(`Schau dir unbedingt unsere BabySteps-App an und verfolge die Entwicklung unseres Babys mit! Die App ist für dich als Mitleser komplett kostenlos: https://deine-app-domain.de/guest/${familyToken}`);
    } 
    else if (type === 'referral') {
        text = encodeURIComponent(`Ich nutze die BabySteps-App für mein Kind und bin total begeistert! Hol dir die App für deine eigene Familie. Nutze diesen Link und wir bekommen beide 10% Rabatt auf Premium! https://deine-app-domain.de/invite/${referralCode}`);
    }
