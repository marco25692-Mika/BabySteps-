...
function initWelcomeLanguageSelection() {
    // Bleibt leer, um doppelte Flaggen zu verhindern
}
...
function renderDashboard() {
    ...
    // Begrüßung (Guten Morgen) komplett entfernt
    if (h) h.innerHTML = ''; 
    if (!c) return; c.innerHTML = '';

    let gridHtml = `<div class="widget-grid">`;

    // Kinder-Kacheln mit kleinen runden Profilbildern
    kids.forEach((kid, i) => {
        ...
        let avatarHtml = kid.avatar 
            ? `<img src="${kid.avatar}" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; margin-bottom: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.08);">` 
            : `<div style="width: 60px; height: 60px; border-radius: 50%; background: #E8DFD5; display:flex; align-items:center; justify-content:center; margin-bottom:12px;"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#8C7B6D" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>`;
    ...
    if (kids.length === 0 && isMomOrDad()) {
        gridHtml += `<div class="widget" onclick="showAddChildScreen()"><div style="width: 60px; height: 60px; border-radius: 50%; background: #E8DFD5; display:flex; align-items:center; justify-content:center; margin-bottom:12px;"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#8C7B6D" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div><div class="widget-title">Kind anlegen</div></div>`;
    }

    const t = translations[currentLang] || translations['de'];
    
    // NEU: Ästhetische, minimalistische Line-Art Icons (SVGs) passend zum Pinterest-Look
    const iconBase = '<svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round">';
    
    let navItems = [
        { id: 'postsScreen', icon: `${iconBase}<path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"></path></svg>`, text: t.menu_diary || 'Tagebuch' },
        { id: 'galleryScreen', icon: `${iconBase}<rect width="18" height="18" x="3" y="3" rx="2" ry="2"></rect><circle cx="9" cy="9" r="2"></circle><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"></path></svg>`, text: t.menu_gallery || 'Galerie' },
        { id: 'milestoneScreen', icon: `${iconBase}<path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"></path><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"></path><path d="M4 22h16"></path><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"></path><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"></path><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"></path></svg>`, text: t.menu_milestones || 'Meilensteine' },
        { id: 'growthScreen', icon: `${iconBase}<polyline points="22 7 13.5 15.5 8.5 10.5 2 17"></polyline><polyline points="16 7 22 7 22 13"></polyline></svg>`, text: t.menu_growth || 'Wachstum' }
    ];

    if (isMomOrDad()) {
        navItems.push(
            { id: 'foodScreen', icon: `${iconBase}<path d="M10 5h4"></path><path d="M12 2v3"></path><path d="M14 5l1 5c1 1 2 2 2 4v5a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2v-5c0-2 1-3 2-4l1-5"></path><path d="M8.5 16h7"></path></svg>`, text: t.menu_food || 'Nahrung' },
            { id: 'sleepScreen', icon: `${iconBase}<path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg>`, text: t.menu_sleep || 'Schlaf' },
            { id: 'cycleScreen', icon: `${iconBase}<path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5C6 11.1 5 13 5 15a7 7 0 0 0 7 7z"></path></svg>`, text: t.menu_cycle || 'Zyklus' },
            { id: 'shoppingScreen', icon: `${iconBase}<path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"></path><path d="M3 6h18"></path><path d="M16 10a4 4 0 0 1-8 0"></path></svg>`, text: t.menu_shopping || 'Einkaufsliste' }
        );
    }

    if (isMomOrDad() || appSettings.pregShare) {
        navItems.push({ id: 'pregnancyScreen', icon: `${iconBase}<path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"></path></svg>`, text: t.menu_pregnancy || 'Schwangerschaft' });
    }

    navItems.push(
        { id: 'wishlistScreen', icon: `${iconBase}<rect x="3" y="8" width="18" height="4" rx="1"></rect><path d="M12 8v13"></path><path d="M19 12v7a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2v-7"></path><path d="M7.5 8a2.5 2.5 0 0 1 0-5A4.8 8 0 0 1 12 8a4.8 8 0 0 1 4.5-5 2.5 2.5 0 0 1 0 5"></path></svg>`, text: t.menu_wishlist || 'Wunschliste' },
        { id: 'gamesScreen', icon: `${iconBase}<circle cx="12" cy="12" r="10"></circle><path d="M8 14s1.5 2 4 2 4-2 4-2"></path><line x1="9" y1="9" x2="9.01" y2="9"></line><line x1="15" y1="9" x2="15.01" y2="9"></line></svg>`, text: t.menu_games || 'Spielecke' },
        { id: 'statsScreen', icon: `${iconBase}<path d="M21.21 15.89A10 10 0 1 1 8 2.83"></path><path d="M22 12A10 10 0 0 0 12 2v10z"></path></svg>`, text: t.menu_stats || 'Statistik' },
        { id: 'profileScreen', icon: `${iconBase}<line x1="4" y1="21" x2="4" y2="14"></line><line x1="4" y1="10" x2="4" y2="3"></line><line x1="12" y1="21" x2="12" y2="12"></line><line x1="12" y1="8" x2="12" y2="3"></line><line x1="20" y1="21" x2="20" y2="16"></line><line x1="20" y1="12" x2="20" y2="3"></line><line x1="1" y1="14" x2="7" y2="14"></line><line x1="9" y1="8" x2="15" y2="8"></line><line x1="17" y1="16" x2="23" y2="16"></line></svg>`, text: t.title_profile || 'Einstellungen' }
    );
...
function initMemoryScreen() {
    ...
    let score = document.getElementById('memoryScore');
    let attEl = document.getElementById('memoryAttempts'); 

    // 1. Diagnose: Fehlt das Spielfeld?
    if (!grid) {
        alert("🚨 Das Spielfeld (memoryGrid) wurde im HTML nicht gefunden! Bitte prüfe, ob die ID in der index.html stimmt.");
        return;
    }
    ...
    try {
        if (typeof allPosts !== 'undefined') {
            Object.keys(allPosts).forEach(d => {
                if (allPosts[d] && Array.isArray(allPosts[d])) {
                    allPosts[d].forEach(pt => {
                        if (pt && pt.media && Array.isArray(pt.media)) {
                            let isVis = typeof isKidVisible === 'function' ? isKidVisible(pt.childIndex) : true;
                            if (isVis) {
                                pt.media.forEach(m => {
                                    let s = (typeof m === 'string') ? m : (m.url || '');
                                    let iV = (typeof m === 'object' && m.type === 'video') || s.includes('video');
                                    if (s && !iV && !aI.includes(s)) aI.push(s);
                                });
                            }
                        }
                    });
        ...
        alert("🚨 Fehler beim Suchen der Bilder: " + e.message);
        return;
    }

    // 2. Diagnose: Zu wenige Bilder?
    if (aI.length < 6) {
        alert(`Das Spiel bricht ab: Es wurden nur ${aI.length} von 6 nötigen Fotos in deinem Tagebuch gefunden.`);
        grid.innerHTML = `<div style="grid-column: span 3; text-align:center; padding: 20px; background:#F4EFEA; border-radius:24px; color:#4A4036; font-weight:700;">Du brauchst mindestens 6 Fotos im Tagebuch für das Spiel! (Aktuell: ${aI.length})</div>`;
        return;
    }

    // Wenn alles klappt, Spiel starten:
    grid.innerHTML = '';
    if(score) score.innerText = "0 / 6 Paare";
    flippedCards = [];
    matchedPairs = 0;
    memoryAttempts = 0; 
    lockBoard = true; 

    let bestScore = localStorage.getItem('memoryHighscore') || '-';
    if(attEl) attEl.innerText = `Versuche: 0 | Bestleistung: ${bestScore}`;

    aI = aI.sort(() => 0.5 - Math.random()).slice(0, 6);
    memoryCards = [...aI, ...aI].sort(() => 0.5 - Math.random());
    let allImgElements = []; 

    memoryCards.forEach((imgSrc) => {
        let card = document.createElement('div');
        card.className = 'widget';
        card.style.cssText = "height: 90px; padding: 0; overflow: hidden; cursor: pointer; transition: 0.3s; background: #FFFFFF; border-radius: 16px; position: relative;";
        card.dataset.img = imgSrc;
        
        let img = document.createElement('img');
        img.src = imgSrc;
        img.style.cssText = "width: 100%; height: 100%; object-fit: cover; opacity: 1; transition: opacity 0.3s; pointer-events: none;";
        
        card.appendChild(img);
        allImgElements.push(img);
        
        card.onclick = function() {
            if (lockBoard || card === flippedCards[0] || img.style.opacity === '1') return;
            ...
            if (flippedCards.length === 2) {
                lockBoard = true;
                memoryAttempts++; 
                if(attEl) attEl.innerText = `Versuche: ${memoryAttempts} | Bestleistung: ${bestScore}`;

                if (flippedCards[0].dataset.img === flippedCards[1].dataset.img) {
                    matchedPairs++;
                    if(score) score.innerText = `${matchedPairs} / 6 Paare`;
                    flippedCards = [];
                    lockBoard = false;
                    
                    if (matchedPairs === 6) {
                        let currentBest = localStorage.getItem('memoryHighscore');
                        if (!currentBest || memoryAttempts < parseInt(currentBest)) {
                            localStorage.setItem('memoryHighscore', memoryAttempts);
                            setTimeout(() => alert(`🎉 NEUER REKORD! Du hast nur ${memoryAttempts} Versuche gebraucht!`), 400);
                        } else {
                            setTimeout(() => alert(`🎉 Gewonnen! Du hast ${memoryAttempts} Versuche gebraucht.`), 400);
                        }
                    }
                } else {
                    setTimeout(() => {
                        flippedCards[0].querySelector('img').style.opacity = '0';
                        flippedCards[0].style.transform = "scale(1)";
                        flippedCards[1].querySelector('img').style.opacity = '0';
                        flippedCards[1].style.transform = "scale(1)";
                        flippedCards = [];
                        lockBoard = false;
                    }, 1000);
                }
    ...
    setTimeout(() => {
        allImgElements.forEach(img => img.style.opacity = '0');
        lockBoard = false;
    }, 2000);
}
...
/* ========================================== */
/* HINTERGRUND-STEUERUNG (NEU & ROBUST)       */
/* ========================================== */

/* ========================================== */
/* SPRACHAUSWAHL (MENÜ ÖFFNEN)                */
/* ========================================== */

function toggleLanguage() {
    // Öffnet das versteckte Menü mit allen 5 Sprachen
    let modal = document.getElementById('quickLangModal');
    if (modal) {
        modal.style.display = 'flex';
    }
}

// Sorgt dafür, dass sich die Flagge oben rechts immer anpasst
function updateTopFlag() {
    const fMap = { de: '🇩🇪', en: '🇬🇧', es: '🇪🇸', it: '🇮🇹', fr: '🇫🇷' };
    const flagBtn = document.querySelector('.top-btn');
    if (flagBtn) {
        flagBtn.innerText = fMap[currentLang] || '🇩🇪';
    }
}

// Verknüpft deine bestehende Sprach-Funktion mit dem Flaggen-Wechsel
const oldSetLanguage = setLanguage;
setLanguage = function(lang) {
    oldSetLanguage(lang); // Ruft deine normalen Übersetzungen auf
    updateTopFlag();      // Ändert das Icon
};

// Setzt die Flagge direkt beim Start der App richtig
window.addEventListener('DOMContentLoaded', updateTopFlag);
